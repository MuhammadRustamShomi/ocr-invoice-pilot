...
def _secret(key: str, default: str = "") -> str:
    """st.secrets first (Streamlit Cloud), then env var, then default."""
    try:
...
# Pull GCP credentials at module level so we can pass them into cached fn
...
# ── Data loading (no st.* calls allowed inside cached functions) ───────────────
@st.cache_data(ttl=30, show_spinner=False)
def _fetch_sheet(sheet_id: str, creds_json: str) -> tuple[list, str]:
    """Fetch all records from Google Sheets. Returns (records, error_str)."""
    try:
        import json
        import gspread
...
def load_results() -> tuple[pd.DataFrame, str]:
    """Return (DataFrame, error_message). Called in main flow — may use st.*."""
    if SHEET_ID and _gcp_info:
        import json
        records, err = _fetch_sheet(SHEET_ID, json.dumps(_gcp_info))
        if err:
            return pd.DataFrame(), err
        return (pd.DataFrame(records) if records else pd.DataFrame()), ""

    if SHEET_ID and not _gcp_info:
        # Local dev: try credentials.json file directly (not cached to avoid stale creds)
        try:
            import gspread
            from google.oauth2.service_account import Credentials
            creds_file = _secret("GOOGLE_CREDENTIALS_FILE", "credentials.json")
            if not Path(creds_file).exists():
                return pd.DataFrame(), f"credentials.json not found at '{creds_file}'"
...
c2.metric("Success Rate", f"{(stats['total_success'] / max(stats['total_processed'], 1)) * 100:.1f}%")
...
if data_error:
    st.warning(f"Could not load data: {data_error}")
elif df.empty:
    if not SHEET_ID:
        st.info(
...
if not df.empty and "Status" in df.columns:
    ...
    if not failed_rows.empty:
        show_cols = [c for c in ["Filename", "Vendor", "Invoice No", "Timestamp"]
